...
export default function PlantPage() {
  ...
  useEffect(() => {
    if (!isInitialized || !plantId) return;
    (async () => {
      try {
  ...
  const save = async (p) => { const u = await updatePlant(plantId, p, user?.id); setPlant(u); setTemp(u); };

  const compress = async (f) => {
    try { const c = await imageCompression(f, { maxSizeMB: 1, maxWidthOrHeight: 1920, useWebWorker: true }); return await imageCompression.getDataUrlFromFile(c); }
    catch { return null; }
  };

  const onMain = async (e) => { const f = e.target.files[0]; if (f?.type.startsWith('image/')) { const u = await compress(f); if (u) await save({ ...temp, mainImage: u }); }};
  const onAdd = async (e) => { const f = e.target.files[0]; if (f?.type.startsWith('image/')) { const u = await compress(f); if (u) await save({ ...temp, images: [...(temp.images || []), u] }); } if (addRef.current) addRef.current.value = ''; };
  const onRemove = async (i) => await save({ ...temp, images: temp.images.filter(x => x !== i) });

  const handleShare = () => { if (!user) { setShowSignInModal(true); return; } setShowShareModal(true); setCopied(false); };
  const copyShareLink = async () => { await navigator.clipboard.writeText(`${window.location.origin}/share/${gardenId}/plant/${plantId}`); setCopied(true); setTimeout(() => setCopied(false), 2000); };

  const onAutofillClick = () => { const d = findData(temp.scientificName); d ? (setAutofillData(d), setShowAutofillModal(true)) : setShowNotFoundModal(true); };
  const onAutofill = async () => { if (!autofillData) return; await save({ ...temp, commonName: autofillData['Common name'] || temp.commonName, bloomTime: autofillData['Bloom time'] || temp.bloomTime, height: autofillData['Height'] || temp.height, sunlight: mapSun(autofillData['Sunlight']) || temp.sunlight, moisture: autofillData['Moisture'] || temp.moisture, hasAutofilled: true }); setShowAutofillModal(false); };
  const onDelete = async () => { await deletePlant(plantId, user?.id); router.push(`/garden/${gardenId}`); };
  ...
  if (!isInitialized || isLoading) return <div className={styles.loading}>Loading...</div>;
  if (!plant) return null;

  return (
    <>
      <div className={styles.container}>
        <PageHeader title={plant.commonName || plant.scientificName || 'Plant'} onBack={() => router.push(`/garden/${gardenId}`)} actions={editing ? <Button variant="success" onClick={async () => { await save(temp); setEditing(false); }}>Save</Button> : <DropdownMenu items={menu} />} />
        <div className={styles.details}>
          ...
          <div className={styles.infoGridWrapper}><div className={styles.infoGrid}>
          ...
          <div className={styles.photosSection}>
            ...
            {temp.images?.length > 0 ? (
              <div className={styles.imageGrid}>
                {temp.images.map((img, i) => (<div key={i} className={styles.photoItem}><img src={img} alt="" className={styles.photo} onClick={() => setSelectedImage(img)} /><button onClick={e => { e.stopPropagation(); onRemove(img); }} className={styles.removeButton}><IoClose size={16} /></button></div>))}
                <button className={styles.addPhotoButton} onClick={() => addRef.current?.click()}><FiPlus size={24} /><span>Add Photo</span><input ref={addRef} type="file" onChange={onAdd} className={styles.fileInput} accept="image/*" /></button>
              </div>
            ) : (<div className={styles.emptyPhotosContainer}><p className={styles.noPhotos}>No additional photos yet.</p><div className={`${styles.emptyPhotosAddButton} ${editing ? styles.visible : ''}`}><button className={styles.addPhotoButton} onClick={() => addRef.current?.click()}><FiPlus size={24} /><span>Add Photo</span><input ref={addRef} type="file" onChange={onAdd} className={styles.fileInput} accept="image/*" /></button></div></div>)}
          </div>
        </div>
      </div>
      ...
      <Modal isOpen={showShareModal} onClose={() => setShowShareModal(false)} title="Share Plant" size="small">
        ...
        <div className={styles.shareLink}><code>{`${typeof window !== 'undefined' ? window.location.origin : ''}/share/${gardenId}/plant/${plantId}`}</code></div>
        <div className={styles.shareButtons}><Button variant="secondary" onClick={() => setShowShareModal(false)}>Close</Button><Button onClick={copyShareLink}>{copied ? 'Copied!' : 'Copy Link'}</Button></div>
      </Modal>
