...
import { buildAutofillImageUrl } from './autofillImages';
import { findData } from './plantAutofill';
...
// Clear autofill image URLs that don't match actual autofill data.
// Prevents broken 404 images from plants autofilled before imageExists() validation.
// Resolves the reference entry exactly the way autofill does (findData:
// case-insensitive scientific name, falling back to common name), so a plant
// whose name differs only in casing from the JSON key doesn't lose a valid image.
const cleanAutofillImages = (plants) => {
  return plants.map(plant => {
    if (!plant.mainImage || !plant.mainImage.includes('autofill_images')) {
      return plant;
    }
    const entry = findData(plant.scientificName, plant.commonName)?.data;
    if (!entry) return { ...plant, mainImage: '' };
    const paths = entry['Autofill image paths'];
    if (!Array.isArray(paths) || !paths[0]) return { ...plant, mainImage: '' };
    const validUrl = buildAutofillImageUrl(paths[0]);
    return plant.mainImage === validUrl ? plant : { ...plant, mainImage: '' };
  });
};
...
const plantFromSupabase = (row) => ({
  ...
  hostedInsects: row.hosted_insects || '',
  plantPrivacy: row.plant_privacy || null,
  notes: row.notes,
  images: row.images || [],
  hasAutofilled: row.has_autofilled,
  autofillSig: row.autofill_sig || null,
});

// Exported so every insert path (including the local -> account migration in
// AuthContext) writes the same columns; a second hand-written column list is
// how plant_type/hosted_insects/autofill_sig silently went missing on migrated
// plants after those columns were added.
export const plantToSupabase = (plant, userId) => ({
  user_id: userId,
  ...
  hosted_insects: plant.hostedInsects || '',
  plant_privacy: plant.plantPrivacy || null,
  notes: plant.notes,
  images: plant.images || [],
  has_autofilled: plant.hasAutofilled || false,
  autofill_sig: plant.autofillSig || null,
});
  ...
  preloadPromise = (async () => {
    try {
      ...
      for (const gardenId of gardenIds) {
        ...
        if (plants) {
          const now = Date.now();
          let plantList = plants.map(plantFromSupabase);
          plantList = cleanAutofillImages(plantList);
...
/**
 * Get just the owner's user id for a garden — a cheap ownership check that
 * doesn't pull the owner profile. Returns null if the garden doesn't exist.
 */
export async function getGardenOwnerId(gardenId) {
  if (!gardenId) return null;

  const cachedGarden = cache.gardenDetails.get(gardenId);
  if (isCacheValid(cachedGarden)) return cachedGarden.data?.userId || null;

  const cachedShared = cache.sharedGardens.get(gardenId);
  if (isCacheValid(cachedShared)) return cachedShared.garden?.userId || null;

  try {
    const result = await safeQuery(() =>
      supabase.from('gardens').select('user_id').eq('id', gardenId).single()
    );
...
export async function getSharedGardenPlants(gardenId) {
  ...
  let plants = (result?.data || []).map(plantFromSupabase);
  plants = cleanAutofillImages(plants);
  const now = Date.now();
...
export async function getSharedGarden(gardenId) {
  ...
  let plants = (plantsResult?.data || []).map(plantFromSupabase);
  plants = cleanAutofillImages(plants);
...
export async function getSharedPlant(plantId) {
  ...
  let plant = plantFromSupabase(result.data);
  const [cleaned] = cleanAutofillImages([plant]);
  plant = cleaned;
  const owner = await getCachedProfile(result.data.user_id);
...
export async function getGardens(userId) {
  if (userId) {
    ...
    // Local storage path
    return (await localforage.getItem('gardens')) || [];
  }
}
...
// Create many plants at once (bulk upload). Returns the created plants in the
// same shape as createPlant. Signed-in users get a single batch insert.
export async function createPlants(plantList, userId) {
  if (!plantList || plantList.length === 0) return [];

  if (userId) {
    const result = await safeQuery(() =>
      supabase.from('plants').insert(plantList.map(p => plantToSupabase(p, userId))).select()
    );

    if (!result?.data) throw new Error('Failed to create plants');

    const newPlants = result.data.map(plantFromSupabase);
    newPlants.forEach(addPlantToCache);
    return newPlants;
  } else {
    const plants = (await localforage.getItem('plants')) || [];
    const newPlants = plantList.map(p => ({ ...p, id: crypto.randomUUID() }));
    await localforage.setItem('plants', [...plants, ...newPlants]);
    return newPlants;
  }
}
    ...
    const result = await safeQuery(() =>
      supabase.from('plants')
        .update({
          ...
          plant_type: updates.plantType || [],
          hosted_insects: updates.hostedInsects || '',
          plant_privacy: updates.plantPrivacy || null,
          notes: updates.notes,
          images: updates.images || [],
          has_autofilled: updates.hasAutofilled || false,
          autofill_sig: updates.autofillSig || null,
        })
        .eq('id', plantId)
        .eq('user_id', userId)
        .select()
...
const LOCAL_PROFILE_VISIBILITY_KEY = 'profileVisibility';
...
const normalizeVisibility = (vis) => ({
  ...
  createdOrder: vis?.createdOrder || [],
  savedOrder: vis?.savedOrder || [],
  recentOrder: vis?.recentOrder || [],
  hiddenAboutBlockIds: vis?.hiddenAboutBlockIds || [],
});

/**
 * Whether a set of profile "about" blocks contains anything worth showing
 * publicly. Returns false for the default/empty state (no blocks, or blocks
 * with only placeholder titles and no real body text or images), so the
 * shared profile can hide the About tab entirely.
 */
export function hasPublicAboutContent(blocks) {
  if (!Array.isArray(blocks)) return false;
  return blocks.some(b =>
    (b.type === 'text' && typeof b.content === 'string' && b.content.trim() !== '') ||
...
export async function getProfileVisibility(userId) {
  if (!userId) {
    const local = await localforage.getItem(LOCAL_PROFILE_VISIBILITY_KEY);
    return normalizeVisibility(local);
  }
  const result = await safeQuery(() =>
    supabase.from('profiles')
  ...
  return normalizeVisibility(result?.data?.profile_visibility);
}

/**
 * Reorder a list of items according to an array of ids.
 * Items whose id appears in `orderIds` are returned first, in that order.
 * Items not in `orderIds` are appended in their original order.
 */
export function applyManualOrder(items, orderIds, getId = (i) => i.id) {
...
export async function updateProfileVisibility(userId, visibility) {
  if (!userId) {
    await localforage.setItem(LOCAL_PROFILE_VISIBILITY_KEY, visibility);
    return;
  }
  const { error } = await supabase
    .from('profiles')
...
// ============ DISPLAY CUSTOMIZATION (plant vs wildlife) ============

export const DEFAULT_DISPLAY = { columns: 4, bgColor: '#f4f4f9', hideBadges: false };

// Plant display settings live at the top level of customization.
export function getPlantDisplay(customization) {
  const c = customization || {};
  return {
    columns: c.columns ?? DEFAULT_DISPLAY.columns,
    bgColor: c.bgColor ?? DEFAULT_DISPLAY.bgColor,
    hideBadges: c.hideBadges ?? DEFAULT_DISPLAY.hideBadges,
  };
}

// Wildlife display settings live under customization.wildlife. Until saved, they
// derive from the plant settings but force columns to 4.
export function getWildlifeDisplay(customization) {
  const c = customization || {};
  if (c.wildlife) return { ...DEFAULT_DISPLAY, ...c.wildlife };
  const plant = getPlantDisplay(customization);
  return { ...plant, columns: 4 };
}
...
export function getHiddenAboutBlockIds(garden) {
  return garden?.customization?.hiddenAboutBlockIds || [];
}
 
// Todo is private by default — only public when explicitly set to false
export function isTodoPrivate(garden) {
  return garden?.customization?.todoPrivate !== false;
}
...
export async function getSharedProfileData(userId) {
  ...
  const createdOrder = vis.createdOrder || [];
  const savedOrder = vis.savedOrder || [];
  const recentOrder = vis.recentOrder || [];
  const hiddenAboutBlockIds = vis.hiddenAboutBlockIds || [];
  ...
  const createdGardens = applyManualOrder(
    allCreated.filter(g => !hiddenCreatedIds.includes(g.id)),
    createdOrder
  );
  ...
  if (visibleSavedIds.length > 0) {
    ...
    savedGardens = applyManualOrder(savedGardens, savedOrder);
  }
  ...
  if (visibleRecentIds.length > 0) {
    ...
    recentGardens = applyManualOrder(recentGardens, recentOrder);
  }
  ...
  return {
    profile: {
      ...
      about_blocks: (profile.about_blocks || []).filter(b => !hiddenAboutBlockIds.includes(b.id)),
    },
    createdGardens,
    savedGardens,
    recentGardens,
...
export async function copyGardenWithPlants(sourceGardenId, newName, newImage, userId, isShared = false) {
  ...
  sourcePlants = cleanAutofillImages(sourcePlants);
  ...
  const createResult = await safeQuery(() =>
    supabase.from('gardens')
      .insert({
        ...
        customization: {
          columns: sourceGarden.customization?.columns || 4,
          bgColor: sourceGarden.customization?.bgColor || '#f4f4f9',
          hideBadges: sourceGarden.customization?.hideBadges || false,
          ...(sourceGarden.customization?.wildlife ? { wildlife: sourceGarden.customization.wildlife } : {}),
        },
      })
      .select()
      .single()
  ...
  if (sourcePlants.length > 0) {
    const plantInserts = sourcePlants.map(p => ({
      ...
      hosted_insects: p.hostedInsects || '',
      notes: p.notes,
      images: p.images || [],
      has_autofilled: p.hasAutofilled || false,
      autofill_sig: p.autofillSig || null,
    }));
