    ...
    criteria: [
      {
        ...
        tooltip: 'Top 30 Host plant for Pollen Specialist Bees',
        check: (entry) => {
...
/** Returns an array of tooltip lines for a badge, e.g. ['Supports specialist bees', 'Supports bumblebees'] */
export function getBadgeTooltip(badgeKey, commonName, scientificName) {
  ...
  const parts = def.criteria
    ...
    .map(c => c.tooltip);
  return parts.length > 0 ? parts : [badgeKey];
}
